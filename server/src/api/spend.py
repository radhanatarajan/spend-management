...
def _apply_filters(stmt, *, month_keys, expense_types, company_codes,
                   oracle_departments, oracle_account_groups, vendors, je_sources,
                   exclude=()):
    """Apply filters to a statement, skipping fields listed in exclude."""
    if month_keys and "month_keys" not in exclude:
        stmt = stmt.where(Spend.month_key.in_(month_keys))
    if expense_types and "expense_types" not in exclude:
        stmt = stmt.where(Spend.expense_type.in_(expense_types))
    if company_codes and "company_codes" not in exclude:
        stmt = stmt.where(Spend.company_code.in_(company_codes))
    if oracle_departments and "oracle_departments" not in exclude:
        stmt = stmt.where(Spend.oracle_department.in_(oracle_departments))
    if oracle_account_groups and "oracle_account_groups" not in exclude:
        stmt = stmt.where(Spend.oracle_account_group.in_(oracle_account_groups))
    if vendors and "vendors" not in exclude:
        stmt = stmt.where(Spend.vendor_name.in_(vendors))
    if je_sources and "je_sources" not in exclude:
        stmt = stmt.where(Spend.je_source.in_(je_sources))
    return stmt
...
def list_spend(
    ...
    stmt = _apply_filters(select(Spend), **filter_kwargs)
...
def get_filter_options(
    ...
    def filtered(exclude):
        return _apply_filters(select(Spend), **fk, exclude=(exclude,))

    months_raw = db.execute(
        _apply_filters(select(Spend.month_key, Spend.month_label), **fk, exclude=("month_keys",))
        .distinct().order_by(Spend.month_key.desc())
    ).all()

    expense_type_vals = db.execute(
        filtered("expense_types").with_only_columns(Spend.expense_type)
        .distinct().order_by(Spend.expense_type)
    ).scalars().all()

    company_code_vals = db.execute(
        filtered("company_codes").with_only_columns(Spend.company_code)
        .distinct().order_by(Spend.company_code)
    ).scalars().all()

    depts_raw = db.execute(
        _apply_filters(select(Spend.oracle_department, Spend.oracle_department_name), **fk, exclude=("oracle_departments",))
        .distinct().order_by(Spend.oracle_department)
    ).all()

    group_vals = db.execute(
        filtered("oracle_account_groups").with_only_columns(Spend.oracle_account_group)
        .distinct().order_by(Spend.oracle_account_group)
    ).scalars().all()

    vendor_vals = db.execute(
        filtered("vendors").with_only_columns(Spend.vendor_name)
        .distinct().order_by(Spend.vendor_name)
    ).scalars().all()

    je_vals = db.execute(
        filtered("je_sources").with_only_columns(Spend.je_source)
        .where(Spend.je_source.is_not(None))
        .distinct().order_by(Spend.je_source)
    ).scalars().all()
