...
def create_project(body: ProjectIdCreate, db: Session = Depends(get_db), _=Depends(require_write)):
...
def update_project(project_id: int, body: ProjectIdUpdate, db: Session = Depends(get_db), _=Depends(require_write)):
...
def delete_project(project_id: int, db: Session = Depends(get_db), _=Depends(require_write)):
    proj = _get_or_404(project_id, db)
